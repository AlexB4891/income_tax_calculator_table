               ...
               modalDialog(
                 ...
                 fluidRow(
                   column(6,
                          textInput(inputId = "nombre", 
                                    label =  "¿Cómo te llamas?", ## AWUI
                                    value = "Don Telesforo")
                   ),
                   column(6,
                          selectInput(inputId = "genero", 
                                      label =  "¿Con que género te identificas?", ## AWUI
                                      choices = c("Masculino","Femenino","Otro"), ## AWUI
                                      selected = "Masculino")) ## AWUI
                 ),
                   with_tippy(
                     numericInput(inputId = "ingreso_lab", 
                                  label =  "¿Cual es tu salario mensual?", ## AWUI
                                  min = 1,
                                  max = 999999,
                                  value = 472),
                     tooltip = "Considera unicamente los ingresos bajo relación de dependencia",  ## AWUI
                     placement = "right"
                   ),
                    
                          with_tippy(
                          sliderInput(inputId = "meses",
                                      label = "Número de meses trabajados:", ## AWUI
                                      min = 1,
                                      max = 12,
                                      value = 12),
                     tooltip = "Número de meses que trabajas con un mismo empleador",  ## AWUI
                     placement = "right"),
                   h2("Declaración de gastos personales"), ## AWUI
                 with_tippy(
                   ...
                   tooltip = HTML("<ul>
                                  ...
                                  En todos los casos señalados en este rubro y en los anteriores, los gastos serán considerados en razón de una sola vivienda."),  ## AWUI
                     placement = "right"),
                 with_tippy(
                 ...
                 footer = tagList(
                   actionButton(inputId = "procesar",
                                label = "Continuar",width = "100%") , ## AWUI
                   size = "l"
...
resumen_impositivo_long <- reactive({
  ...
  tabla_resumen_impositivo() %>% 
    ...
    mutate(name = case_when(
      name == "fraccion_basica" ~ "Fracción básica (c)",   ## AWUI
      name == "impuesto_fraccion" ~ "Impuesto sobre la fracción básica (d)",  ## AWUI
      name == "impuesto_excedente" ~ "Tasa sobre el excedente (e)", ## AWUI
      name == "cuota" ~ "Cuota deducible (f)"## AWUI
    )) %>% 
...
output$tabla_argumentos <- render_gt({
  ...
  bind_cols(tabla_ingresos(), 
    ...
    cols_label(
      ingresos = md("**Ingresos (a)**"), ## AWUI
      gastos_clase = "",
      gastos_valor = md("**Valor**"), ## AWUI
      name = md("**Concepto**"), ## AWUI
      value = md("**Valor**")) %>% ## AWUI
    sub_missing(
      columns = c(ingresos,
                  name,
    ...
    tab_spanner(label =  md("**Gastos personales (b)**"), ## AWUI
                columns = c(gastos_clase,
                            gastos_valor)) %>%
    tab_spanner(label =  md("**Resumen impositivo**"), ## AWUI
                columns = c(name,
                            value))  %>%
    fmt_currency(
...
output$tabla_resultado <- render_gt({
  ...
  calculo_impuesto() %>%
    ...
    cols_label(
      ...
      tie = md("**¿Cuanto representa el IR en mis ingresos?**")) %>% ## AWUI
    fmt_currency(
